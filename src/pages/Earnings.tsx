...
import { formatWalletBalance, parseIdrInput } from '../utils/currency';
...
export function Earnings() {
  ...
  const handleWithdraw = () => {
    const result = requestMockWithdrawal(parseIdrInput(withdrawAmount));
  ...
  return (
    <AppLayout>
      ...
      <p className={shared.pageSubtitle}>
        Mock tutor earnings and withdrawal in rupiah (demo only).
      </p>
      ...
      >
        <div className={shared.card} style={{ padding: 20 }}>
          ...
          <p style={{ fontSize: 28, fontWeight: 800, margin: '8px 0 0' }}>
            {formatWalletBalance(earnings.available)}
          </p>
        </div>
        <div className={shared.card} style={{ padding: 20 }}>
          <p className={shared.empty} style={{ margin: 0 }}>Pending</p>
          <p style={{ fontSize: 28, fontWeight: 800, margin: '8px 0 0' }}>
            {formatWalletBalance(earnings.pending)}
          </p>
        </div>
        <div className={shared.card} style={{ padding: 20 }}>
          <p className={shared.empty} style={{ margin: 0 }}>Lifetime</p>
          <p style={{ fontSize: 28, fontWeight: 800, margin: '8px 0 0' }}>
            {formatWalletBalance(earnings.lifetime)}
          </p>
        </div>
      </div>
      ...
      <div className={shared.card} style={{ padding: 24, maxWidth: 420 }}>
        ...
        <div className={shared.formGroup}>
          <label className={shared.label}>Amount (rupiah)</label>
          <input
            type="text"
